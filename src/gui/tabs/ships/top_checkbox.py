...
class TopCheckboxes(QWidget):
    ...
    def init_dropdowns(self):
        ...
        self.add_dropdown_on_text("LEVEL", SCONST.level_select, self.proxy.set_level_filter, 0, 9, 2)
        self.add_dropdown_on_text("LOCK", SCONST.lock_select, self.proxy.set_lock_filter, 0, 11, 2)
        self.add_dropdown_on_text("MARRY", SCONST.married_select, self.proxy.set_marry_filter, 0, 13, 2)
        self.add_dropdown_on_text("MOD.", SCONST.mod_select, self.proxy.set_mod_filter, 0, 15, 2)
    ...
    def init_ship_boxes(self):
        ...
        for k, v in enumerate(first_row_types):
            ...
            self.first_boxes[k].stateChanged.connect(lambda _, _b=self.first_boxes[k]: self.proxy.set_checkbox_filter(_b))
    ...
    def _clear_filters(self):
        ...
        self.proxy.set_level_filter(None)
        self.proxy.set_lock_filter(None)
        self.proxy.set_marry_filter(None)
        self.proxy.set_mod_filter(None)
        ...
        for b in self.second_boxes:
            b.setChecked(False)
        self.proxy.set_checkbox_filter(None)
